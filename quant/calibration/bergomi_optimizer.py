...
class BergomiOptimizer:
    ...
    def calibrate_grid_search(self) -> dict:
                ...
                for rho in rho_grid:
                    ...
                    if valid.sum() >= 3:
                        ...
                        if rmse < best_rmse:
                            ...
                            print(f"   [{tested}/{total_combos}] New best: H={H:.2f}, eta={eta:.1f}, rho={rho:.1f} -> RMSE={rmse*100:.2f}%")
        ...
        print(f"   eta (vol-vol)= {best_params['eta']:.3f}")
        print(f"   rho (correl) = {best_params['rho']:.3f}")
        print(f"   xi0 (ATM var)= {best_params['xi0']:.4f} (sigma={np.sqrt(best_params['xi0'])*100:.1f}%)")
    ...
    def calibrate(self, method: str = 'grid',
        ...
        print(f"   Initial: H={x0[0]:.3f}, eta={x0[1]:.2f}, rho={x0[2]:.2f}, xi0={x0[3]:.4f}")
        ...
        print(f"   eta (vol-vol)= {eta_opt:.3f}")
        print(f"   rho (correl) = {rho_opt:.3f}")
        print(f"   xi0 (ATM var)= {xi0_opt:.4f} (sigma={np.sqrt(xi0_opt)*100:.1f}%)")

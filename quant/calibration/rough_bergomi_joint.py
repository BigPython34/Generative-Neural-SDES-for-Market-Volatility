...
def _compute_vix_from_window(v_window, dt):
    ...
    vix_sq = integrated / actual_tau                     # E[VIX²] by path
    return 100.0 * jnp.sqrt(jnp.mean(vix_sq))    # E[VIX] ≈ √(E[VIX²]) with convexity adjustment
...
def vix_term_structure_loss(model_vix, market_vix, weights=None):
    ...
    for tau in sorted(common):
        if weights and tau in weights:
            w = weights[tau]
        else:
            # maturity ponderated
            w = np.sqrt(tau / 365.0)
        mkt = market_vix[tau]
        diff = (model_vix[tau] - mkt) / max(mkt, 1.0)
        loss += w * diff ** 2
        total_w += w
...
class JointCalibrator:
    ...
    2. Grid search over (η, ρ) with JAX-accelerated MC
    ...
    def __init__(
        ...
        eta_prior: float = 1.9,
        moneyness_range: tuple = (-0.5, 0.5),  # Wider to capture deeper wings (up to ~30-40% moneyness)
        min_dte: int = 7,
        max_dte: int = 360,
        max_strikes_per_maturity: int = 20,
    ...
    def evaluate(self, eta, rho, xi0_curve, spx_slices, vix_targets,
                 spot, r, T_max, n_steps, key=None):
        """
        Evaluate joint loss at a single (η, ρ) point.
        ...
        model = ExtendedRoughBergomi(
            H=self.H_prior, eta=eta, rho=rho, xi0_curve=xi0_curve,
        ...
        SPX_SCALE  = 1.0 / (0.005 ** 2)    # ~40000
        VIX_SCALE  = 1.0 / (0.03  ** 2)    # ~1111
        MART_SCALE = 1.0 / (0.005 ** 2)    # ~40000

        # ── Total ──
        total = (
            self.lambda_spx  * spx_loss_val  * SPX_SCALE
            + self.lambda_vix  * vix_loss_val  * VIX_SCALE
            + self.lambda_mart * mart_loss_val * MART_SCALE
        )
    ...
    # ─────────────────────────────────────────────────────
    def _adaptive_steps(self, T_max, H, base=252):
    # Biais ≈ O(dt^(H+0.5)) Bayer et al. 2016 Appendix B
        scale = max(1.0, (0.15 / max(H, 0.01)) ** 0.5)
        return max(100, int(T_max * base * scale))
    
    def calibrate(self, market_data, H: float=0.07, xi0_curve=None, quick=False):
        """
        ...
        n_steps = self._adaptive_steps(T_max, H)
        dt = T_max / n_steps
        ...
        # ── Stage 2: Grid search ──
        if self.verbose:
            print("\n  [Stage 2] Grid search over (η, ρ)...")

        if quick:
            eta_grid = [0.8, 1.2, 1.8, 2.5]
            rho_grid = [-0.95, -0.85, -0.70]
        else:
            eta_grid = [1.6, 1.8, 2.0, 2.3, 2.6, 2.8, 3]
            rho_grid = [-0.96, -0.95, -0.90, -0.85, -0.80, -0.70, -0.60]

        total_combos =len(eta_grid) * len(rho_grid)

        if self.verbose:
            print(f"    {total_combos} combinations "
                  f"({len(eta_grid)} η × {len(rho_grid)} ρ)")
        ...
        try:
            _ = self.evaluate(
                H, eta_grid[0], rho_grid[0], xi0_curve,
        ...
        if self.verbose:
            ...
            print(f"η={eta_best:.2f} ρ={rho_best:.2f} "
                  f"(L={best_loss:.6f})")
        ...
        eta_best, rho_best
        BOUNDS_ETA = (eta_best-0.5, eta_best+0.5)
        BOUNDS_RHO = (rho_best-0.1, rho_best+0.1)

        if self.verbose:
            print(f"\n [Stage 3] Local refinement (bounded Nelder-Mead)...")
            print(f" η∈{BOUNDS_ETA}, ρ∈{BOUNDS_RHO}")
        ...
        def objective(x):
            ...
            if (eta < BOUNDS_ETA[0] or eta > BOUNDS_ETA[1] or
        ...
        eta_opt = np.clip(local_result.x[0], *BOUNDS_ETA)
        rho_opt = np.clip(local_result.x[1], *BOUNDS_RHO)
        n_local_evals = eval_count[0]

        if self.verbose:
            improved = local_result.fun < best_loss
            print(f"η={eta_opt:.3f} ρ={rho_opt:.3f} "
        ...
        final = self.evaluate(eta_opt, rho_opt, xi0_curve,
            spx_slices, vix_targets,

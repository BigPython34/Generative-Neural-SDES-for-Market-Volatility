    ...
    def get_full_surface(self, max_dte: int = 360, min_volume: int = 1,
        ...
        for exp_str in expirations:
            ...
            try:
                ...
                print(f"  Calls: {len(chain.calls)} rows, Puts: {len(chain.puts)} rows")
                ...
                print(f"  Combined options before filter: {len(df)}")
                ...
                print(f"  Exception for expiration {exp_str}: {e}")
                continue
        if len(all_data) < 1:
            print("WARNING: No options data loaded for any expiration!")
        elif sum(len(df) for df in all_data) < 10:
            print(f"WARNING: Very few options loaded in total: {sum(len(df) for df in all_data)}")

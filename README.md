...
|:--|:--|:--|
| Self-similarity | B^H_ct =^d c^H B^H_t for all c > 0 | Scale invariance of increments |
| Stationary increments | B^H_(t+τ) − B^H_t =^d B^H_τ for all t | Time-shift invariance |
| Variance scaling | Var(B_t^H) = t^(2H) | Power-law variance growth |
| Increment correlation | Corr(ΔB_1^H, ΔB_2^H) < 0 when H < 1/2 | Anti-persistence (roughness) |
...
log V_t = log ξ_0 + η W^H_t − (1/2) η² t^(2H)

```latex
\log V_t = \log \xi_0 + \eta\,\hat{W}^H_t - \tfrac{1}{2}\eta^2\,t^{2H}
```

where W^H_t = sqrt(2H) · ∫₀ᵗ (t−s)^(H−1/2) dW_s is the **Riemann–Liouville fBM** (Volterra kernel representation). The correction term −(1/2)η² t^(2H) ensures E[V_t] = ξ_0 (the forward variance curve is preserved under the risk-neutral measure).
...
L = Σ_k α_k² σ_k² − λ(Σ_k α_k − 1)

```latex
\mathcal{L} = \sum_k \alpha_k^2\sigma_k^2 - \lambda\!\left(\sum_k\alpha_k - 1\right)
```

∂L/∂α_k = 2 α_k σ_k² − λ = 0  ⟹  α_k = λ/(2σ_k²) ∝ 1/σ_k²

The constraint Σ_k α_k = 1 gives α_k = (1/σ_k²) / Σ_j (1/σ_j²), i.e., inverse-variance weighting. The resulting minimum variance is:
...
*(Terms: Delta, Gamma, Vega, Theta, Cross/Vanna, Convexity/Volga, Rho)*
...
dX_t = κ(θ − X_t) dt + f_θ(S_{0,t}, X_t) dt + g_θ(S_{0,t}, X_t) dW_t + J·dN_t (Jumps optional)

*(Terms: OU Prior, Neural Drift, Neural Diffusion, optional Jumps)*
...
X_t = η·W^H_t − (1/2)η²·Var[W^H_t] + ∫₀ᵗ f_θ(S_{0,s}, X_s) ds + ∫₀ᵗ g_θ(S_{0,s}, X_s) dW_s
...
- $S_{0,t} \in T^{(M)}(R^2)$ is the running path signature of $(t, X)$ up to order $M \in \{2,3,4\}$ (configurable), computed via **exact Chen's identity**
